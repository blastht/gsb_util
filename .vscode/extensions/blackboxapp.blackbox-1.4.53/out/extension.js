...
function activate(_) {	
	...
	if (version_control_feature_enabled) {
		versionControl = new VersionControl(_);

		const versionedFilesProvider = new VersionedFilesProvider(versionControl);

		// Register the tree view
		const treeView = vscode.window.createTreeView('versionedFiles', {
			treeDataProvider: versionedFilesProvider
		});

		// Add refresh command
		let refreshCommand = vscode.commands.registerCommand('extension.refreshVersionedFiles', () => {
			versionedFilesProvider.refresh();
		});

		_.subscriptions.push(treeView, refreshCommand);

		// Command to save the current version
		let saveVersionCommand = vscode.commands.registerCommand('extension.saveVersion', () => {
			const editor = vscode.window.activeTextEditor;
			if (editor) {
				versionControl.saveVersion(editor.document.uri);
			}
		});

		_.subscriptions.push(saveVersionCommand);

		let clearAllVersionsCommand = vscode.commands.registerCommand('extension.clearAllVersions', () => {
			versionControl.clearStorage();
			versionedFilesProvider.refresh();
			vscode.window.showInformationMessage('All versions have been cleared.');
		});
	
		_.subscriptions.push(clearAllVersionsCommand);

		let viewVersionDiffCommand = vscode.commands.registerCommand('extension.viewVersionDiff', async (versionItem) => {
			try {
				if (!versionItem || !versionItem.fileUri) {
					throw new Error('Invalid version item or missing file URI');
				}

				const fileUriString = versionItem.fileUri.toString();
				const versions = versionControl.getAllVersions(fileUriString);

				if (!versions || versions.length === 0) {
					throw new Error('No versions available for this file');
				}

				const currentContent = versionItem.content;
				const previousContent = versionItem.index > 0 ? versions[versionItem.index - 1] : "";

				// Get language ID
				const document = vscode.workspace.textDocuments.find(
					doc => doc.uri.toString() === fileUriString
				);
				const languageId = document?.languageId || 'plaintext';

				// Create the documents with the contents
				...
				await vscode.commands.executeCommand('vscode.diff',
					previousDoc.uri,
					currentDoc.uri,
					`Version ${versionItem.index} ↔ Version ${versionItem.index + 1}`
				);
			} catch (error) {
				console.error('Error in viewVersionDiff command:', error);
				vscode.window.showErrorMessage('Error viewing diff: ' + error.message);
			}
		});

		_.subscriptions.push(viewVersionDiffCommand);

		// Command to view versions
		let viewVersionsCommand = vscode.commands.registerCommand('extension.viewVersions', async (uri) => {
			try {
				...
				// Get versions and check if they exist
				const versions = versionControl.getAllVersions(uriString);
				if (!versions || versions.length === 0) {
					vscode.window.showInformationMessage('No versions available for this file.');
					return;
				}

				// Create a quick pick list of versions
				const versionItems = versions.map((content, index) => ({
				...
				if (selectedVersion) {
					// Show diff view
					const selectedIndex = selectedVersion.index;
					const selectedContent = selectedVersion.content;
					const previousContent = selectedIndex > 0 ? versions[selectedIndex - 1] : "";

					// Get current editor's language ID
					const languageId = vscode.window.activeTextEditor?.document?.languageId || 'plaintext';

					// Create the documents with the contents
					const [selectedDoc, previousDoc] = await Promise.all([
						vscode.workspace.openTextDocument({
					...
					await vscode.commands.executeCommand('vscode.diff',
						previousDoc.uri,
						selectedDoc.uri,
						`Version ${selectedIndex} ↔ Version ${selectedIndex + 1}`
					);
				}
			} catch (error) {
				console.error('Error in viewVersions command:', error);
				vscode.window.showErrorMessage('Error viewing versions: ' + error.message);
			}
		});

		_.subscriptions.push(viewVersionsCommand);

		// Listen for document saves to automatically save versions
		vscode.workspace.onDidSaveTextDocument(document => {
			versionControl.saveVersion(document.uri);
			versionedFilesProvider.refresh();
		});

		function restoreVersion(uri, version) {
			const editor = vscode.window.activeTextEditor;
			if (editor && editor.document.uri.toString() === uri.toString()) {
				editor.edit(editBuilder => {
					...
					editBuilder.replace(fullRange, version);
				});
			} else {
				vscode.window.showErrorMessage('Please open the file you want to restore.');
			}
		}

		_.subscriptions.push(
			vscode.commands.registerCommand('extension.restoreVersion', async (versionItem) => {
				try {
					// Check if versionItem exists and has the required properties
					if (!versionItem || !versionItem.fileUri || !versionItem.content) {
						throw new Error('Invalid version data');
					}
		
					// Convert URI if it's a string
					let fileUri = versionItem.fileUri;
					...
					await editor.edit(editBuilder => {
						...
						editBuilder.replace(fullRange, versionItem.content);
					});
		...
		const statusBarVersionControl = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Left);
		statusBarVersionControl.text = "$(versions) Version Control"; // Adds git icon
		statusBarVersionControl.tooltip = "Open Version Control Explorer";
		statusBarVersionControl.command = 'workbench.view.extension.version-control-explorer'; // Command to open the view
		statusBarVersionControl.show();

		// Add to subscriptions to properly dispose
		_.subscriptions.push(statusBarVersionControl);
	}
